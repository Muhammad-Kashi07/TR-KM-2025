...
%%%%%%% Aquí faria falta una mica d'introducció sobre quin era l'objectiu dels experiments. Indicar que faries tres ... i avançar que a més dels resultats obtinguts la teva recerca aporta una sèria de recomanacions per fer servir les tires i obtenir els resultats amb més fiabilitat.
...
Es mostren els resultats obtinguts al \textit{Appendix}~\ref{s:A1}, a la taula ~\nameref{tab:comparacio_dades2}.

Com podem observar a la taula~ \nameref{tab:comparacio_dades2}, la majoria de paràmetres no es detecten en aquesta mostra d’aigua. Tot i això, els que sí que hi apareixen presenten una gran diferència entre les dues tires: la que ha estat exposada a la llum i la que no.
...
Les taules es poden veure al \textit{Appendix}~\ref{s:A1}~ ~\nameref{tab:comparacio_aigua_5s} i~\nameref{tab:comparacio_aigua_120s}
